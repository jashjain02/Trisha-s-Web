...
const ACTIVITY_COLORS: Record<string, string> = {
  wish_claimed: 'bg-blue-100 text-blue-600',
  wish_fulfilled: 'bg-emerald-100 text-emerald-600',
  wish_created: 'bg-pink-100 text-pink-dark',
  points_earned: 'bg-amber-100 text-amber-600',
  joined: 'bg-purple-100 text-purple-600',
}
    ...
    return (
      <div className="space-y-3">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-white/50 animate-pulse">
        ...
        {activities.map((activity, i) => {
          ...
          return (
            <motion.div
              ...
              className="flex items-start gap-3 p-3 rounded-xl bg-white/60 border border-white/80 hover:bg-white/80 transition-colors"
            >
